 ...
 */
import javax.swing.*;
public class HeuristicPerformanceMonitor extends Dump {
  int phase;
  int count;
  Vector phaseStorer;
  ...
  public HeuristicPerformanceMonitor(Pool pool,JTextArea jta,int jobsPerPhase) {
  ...
  public Thread calculateXecutionTime(){
    this.count++;
  HTotalExecutionTimeCalculator thread=new HTotalExecutionTimeCalculator(this.threadPool.getQueueOut(),this.phaseStorer,jobsHistoryStorer,this.threadPool.getCurrentWorkers(),this.jta,jobsPerPhase,this.threadPool,count);
//***********************************************************
 // if(count==20) count=0;
  thread.start();
  return thread;
...
HeuristicPerformanceChecker checker=new HeuristicPerformanceChecker(t,jta,this.phaseStorer,this.threadPool);
    checker.start();
