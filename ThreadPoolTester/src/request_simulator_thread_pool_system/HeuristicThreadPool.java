...
public class HeuristicThreadPool implements Pool
{

  private HeuristicPerformanceMonitor performanceMonitor;
...
return this.queueOut.size();
     ...
     public synchronized void shrinkPoolSize(int numberOfThreads){
       if(this.workers.size()>3)
      for (int i=1; i<=numberOfThreads; i++) {
       // this.workers.removeElementAt(1);
       PoolWorker pw=(PoolWorker) this.workers.firstElement();
    try{   pw.interrupt();}catch(Exception e){}
       this.workers.remove(pw);
      }
     ...
     return this.model;
       ...
       public HeuristicThreadPool(int initialThreads,JTextArea jta,FrequencyHolder frequencyHolder,DefaultTableModel model,DefaultTableModel model2)//,PhaseTabPanel phasePanel)
     {
      ...
      //  this.jobsPerPhase=12;
       ...
       this.performanceMonitor=new HeuristicPerformanceMonitor(this,jta,jobsPerPhase);
...
public synchronized Vector getWorkerPool(){
  return this.workers;
     ...
     private class PoolWorker extends Thread {
       HeuristicThreadPool pool;

         public PoolWorker(HeuristicThreadPool pool){
           setDaemon(true);
         this.pool=pool;
         }
         ...
         public PoolReducer(PoolWorker pw,HeuristicThreadPool pool,java.util.Timer timer){
           poolWorker=pw;
           this.pool=pool;
           this.timer=timer;
         }
