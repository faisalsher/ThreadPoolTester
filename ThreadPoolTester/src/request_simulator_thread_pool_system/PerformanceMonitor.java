 ...
 */
import javax.swing.*;
public class PerformanceMonitor extends Dump { //extend Dump for polymorphism b/c Main need Jobs and phase history of 3 pool implementations
  int phase;
  int count;
  Vector phaseStorer;
  ...
  public PerformanceMonitor(ThreadPool pool,JTextArea jta,int jobsPerPhase) {
  ...
  public Thread calculateXecutionTime(){
    this.count++;
  TotalExecutionTimeCalculator thread=new TotalExecutionTimeCalculator(this.threadPool.getQueueOut(),this.phaseStorer,jobsHistoryStorer,this.threadPool.getCurrentWorkers(),this.jta,jobsPerPhase,this.threadPool,count);
//***********************************************************
 // if(count==20) count=0;
  thread.start();
  return thread;
...
PerformanceChecker checker=new PerformanceChecker(t,jta,this.phaseStorer,this.threadPool);
    checker.start();
