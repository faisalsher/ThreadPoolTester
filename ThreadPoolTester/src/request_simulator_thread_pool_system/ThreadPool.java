...
public class ThreadPool implements Pool {
  ...
  public int getTotalCompletedJobs(){
    synchronized(this.jobCompletionCount){
...
return this.queueOut.size();
     ...
     public synchronized void growPoolSize(int frequency){
    ...
    while(this.workers.size()<frequency)
    {
      PoolWorker pw=new PoolWorker(this);
         this.workers.addElement(pw);
         pw.start();
     ...
     return this.model;
       ...
       public ThreadPool(int initialThreads,JTextArea jta,FrequencyHolder frequencyHolder,DefaultTableModel model,DefaultTableModel model2)//,PhaseTabPanel phasePanel)
     {
       ...
       this.performanceMonitor=new PerformanceMonitor(this,jta,jobsPerPhase);
...
public synchronized void execute(Runnable r) {
     ...
     private class PoolWorker extends Thread {
       ThreadPool pool;
                ...
                synchronized (queue) {
                 ...
                 try {
                   ...
                   if (jobCompletionCount.getJobCount() == jobsPerPhase) {
                     ...
                     // Following line of code must be commented in Little Fbos
         ...
         public PoolReducer(PoolWorker pw,ThreadPool pool,java.util.Timer timer){
           poolWorker=pw;
           this.pool=pool;
           this.timer=timer;
         }
